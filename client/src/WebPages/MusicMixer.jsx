...
function MusicMixer() {
  ...
  const handleSearch = async (e) => {
      ...
      sessionStorage.setItem('musicMixerSearch', JSON.stringify({
        query: searchQuery,
        page: 1
      }));
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'userPlaylists' && e.newValue) {
        try {
          ...
          setPlaylists(updatedPlaylists);
          console.log('Updated playlists from storage event in MusicMixer:', updatedPlaylists.length);
        } catch (error) {
          console.error('Error parsing updated playlists from storage in MusicMixer:', error);
        }
      }
    };
    ...
    const handlePlaylistUpdate = (event) => {
      ...
      setPlaylists(updatedPlaylists);
      console.log('Updated playlists from custom event in MusicMixer:', updatedPlaylists.length);
    };
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className={`min-h-[50vh] flex flex-col bg-gray-50 ${
            ...
            <div className="p-6 bg-white border-b">
                  ...
                  {showSortDropdown && (
                    <div className="absolute right-0 mt-2 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                      <div className="py-2">
                        ...
                        <button
                          onClick={() => { setSortOption('popularity-high'); setShowSortDropdown(false); }}
                          className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortOption === 'popularity-high' ? 'bg-gray-50 text-blue-600' : 'text-gray-700'}`}
                        >
                          Highest Popularity (Default)
                        </button>
                        <button
                          onClick={() => { setSortOption('popularity-low'); setShowSortDropdown(false); }}
                          className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortOption === 'popularity-low' ? 'bg-gray-50 text-blue-600' : 'text-gray-700'}`}
                        >
                          Lowest Popularity
                        </button>
                        <button
                          onClick={() => { setSortOption('title-asc'); setShowSortDropdown(false); }}
                          className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortOption === 'title-asc' ? 'bg-gray-50 text-blue-600' : 'text-gray-700'}`}
                        >
                          Song Title (A-Z)
                        </button>
                        <button
                          onClick={() => { setSortOption('artist-asc'); setShowSortDropdown(false); }}
                          className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${sortOption === 'artist-asc' ? 'bg-gray-50 text-blue-600' : 'text-gray-700'}`}
                        >
                          Artist (A-Z)
                        </button>
              ...
              {tracks.length > 0 && (<span className="text-gray-600 text-sm ml-4">for '<span className="italic">{searchQuery}</span>'</span>)}
            </div>
              ...
              <div className="grid grid-cols-5 gap-6 mb-8">
                {tracks.map((track, index) => (
                  <div 
                    ...
                    style={{animationDelay: `${index * 0.1}s`, animationPlayState: isLoading ? 'paused' : 'running'}}
            ...
            {/* pagination controls */}
            {totalTracks > 0 && (
                <div className={`fixed left-0 right-0 pagination-slide-up z-40 ${
